...

{
    ...
    {
        add_action('wpcf7_init', [$this, 'cf7vb_add_form_tag_timezone']);
        add_filter('wpcf7_validate_timezone', [$this, 'cf7vb_timezone_validation_filter'], 10, 2);
        add_filter('wpcf7_validate_timezone*', [$this, 'cf7vb_timezone_validation_filter'], 10, 2);
        add_action('wpcf7_admin_init', [$this, 'cf7vb_add_tag_timezone'], 20);
    }

    // cf7vb_add_form_tag_timezone

    public function cf7vb_add_form_tag_timezone()
    {
        wpcf7_add_form_tag(
            array('timezone', 'timezone*'),
            [$this, 'cf7vb_timezone_form_tag_handler'],
            array('name-attr' => true)
        );
    }

    // cf7vb_timezone_form_tag_handler
    public function cf7vb_timezone_form_tag_handler($tag)
    {
        if (empty($tag->name)) {
            return '';
        }
        ...
        if (in_array($tag->basetype, array('timezone', 'timezone*'))) {
        ...
        $countrylist = $this->cf7vb_time_zone_list($attsname, $atts);
    ...
    /**
     * show all Country Time Zone 
     */

    public function cf7vb_time_zone_list($attsname, $atts)
    {
        // write a code here
        $timezones = array(
        ...
        $html .= '<option value="" selected disabled>' . esc_html__('Select a Time Zone', 'cf7vb') . '</option>';
    ...
    public function cf7vb_timezone_validation_filter($result, $tag)
    {
        $type = $tag->type;
        $name = $tag->name;
    ...
    public function cf7vb_add_tag_timezone()
    {
        // write a code here
        $tag_generator = WPCF7_TagGenerator::get_instance();
        $tag_generator->add('timezone', __('TimeZone drop-down', 'cf7vb'), [$this, 'cf7vb_tag_generator_timezone']);
    }

    public function cf7vb_tag_generator_timezone($contact_form, $args = '')
    {
        $type = 'timezone';
        $description = __("Generate a form-tag for a Time Zone list depend your Conuntry.", 'cf7vb');
?>
